...
const STYLE_ID = "claude-export-spinner-styles";
const BUTTON_CLASS = "claude-reexport-action";

function injectSpinnerStyles(plugin: Plugin) {
...
function removeAllInjectedButtons(app: App) {
...
export function setupExportButton(
  ...
  injectSpinnerStyles(plugin);
  plugin.register(() => removeAllInjectedButtons(app));
  ...
  const onLeafChange = (leaf: WorkspaceLeaf | null) => {
    removeAllInjectedButtons(app);
    if (!leaf) return;
    if (pendingTimer !== null) clearTimeout(pendingTimer);
    pendingTimer = setTimeout(() => {
      pendingTimer = null;
      injectButton(leaf, app, getSettings);
    }, 300);
  };
...
function injectButton(
  ...
  // Avoid duplicates
  if (actionsBar.querySelector(`.${BUTTON_CLASS}`)) return;

  const btn = document.createElement("a");
  btn.className = `view-action clickable-icon ${BUTTON_CLASS}`;
